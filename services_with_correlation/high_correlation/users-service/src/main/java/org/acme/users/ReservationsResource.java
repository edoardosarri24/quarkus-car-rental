...
public class ReservationsResource {
    ...
    public RestResponse<TemplateInstance> create(
        ...
        startChoice.pass();
        startparallel.pass();
        double busywaitTime = new ExponentialSampler(new BigDecimal(1).divide(new BigDecimal(7),java.math.MathContext.DECIMAL128)).getSample().doubleValue();
        boolean choice = busywaitTime < 7.0 ? true : false;
        long busyWaitTimeNs = (long) (busywaitTime * 1.0E+6);
        long startTime = System.nanoTime();
        while (System.nanoTime() - startTime < busyWaitTimeNs) {
            // Busy wait
        }
        client.make(reservation, choice);
