...
\begin{itemize}
    ...
    \item Non-parametric Bootstrap \\
        Si tratta di un approccio data-driven~\cite{breheny2016bootstrap}: invece che usare formule teoriche che solitamente fanno assunzioni come il tipo di distribuzione o il numero di campioni infiniti, in questo caso si usano i dati per studiare la variabilità dei dati. La tecnica dice di campionare con reinserimento (bootstrapping) dalla distribuzione campionaria, con l'idea che i dati siano la migliore rappresentazione della distribuzione reale (non nota) da cui essi derivano. Dopo aver campionato $N$ volte si ottiene una Empirical Cumulative Distribution Function; se ripetiamo il processo molte volte allora possiamo definire le bande di confidenza. Il risultato sono bande di dimensione costante e, siccome non si usano formule matematiche ma i dati stessi, meno conservative di altre tecniche come DKW.
\end{itemize}

\subsection{Risultati}
Il nostro numero di campioni è stato abbastanza elevato da produrre una CDF campionaria abbastanza precisa. I risultati, che mostrano questa precisione, sono ottenuti richiedendo una confidenza del 99\%.
\begin{itemize}
    \item Le bande ottenuto con il metodo di Clopper-Pearson sono mostrate in Figura~\ref{fig:correlation0_clopper_perason}.
    \item Le bande ottenuto con il metodo DKW sono mostrate in Figura~\ref{fig:correlation0_dkw}.
    \item Le bande ottenuto con il metodo del bootstrapping sono mostrate in Figura~\ref{fig:correlation0_bootstrap}.
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=.9\textwidth]{images/6-analisi/compare_e2e_distribution_correlation0_clopperPearson.pdf}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=.9\textwidth]{images/6-analisi/compare_e2e_distribution_correlation0_DKW.pdf}
        \caption{Simultaneus con Dvoretzky-Kiefer-Wolfowitz (DKW)}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=.9\textwidth]{images/6-analisi/compare_e2e_distribution_correlation0_bootstrapping.pdf}
        \caption{Simultaneus con bootstrapping.}
        \label{fig:correlation0_bootstrap}
    \end{subfigure}
    \caption{Bande di confidenza.}
\end{figure}